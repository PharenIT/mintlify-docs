...
For local testing, choose **Custom URL environment** and set **baseUrl** to `http://localhost:8000`. Include `http://` or `https://` so the playground can build a valid URL. Playground requests are sent directly from your browser, so your local backend must be running and allow requests from the docs site.
