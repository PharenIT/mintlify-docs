...
Each endpoint page has a **Try it** panel. Set **Authorization** to your Bearer token, then choose the server you want to test.

For local testing, choose **Custom HTTP environment** and set **host** to `localhost:8000`. Do not include `http://` in **host** because the server option already adds it. Playground requests are sent directly from your browser, so your local backend must be running and allow requests from the docs site.
